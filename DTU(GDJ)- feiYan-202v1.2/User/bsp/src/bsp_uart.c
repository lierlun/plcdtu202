...
void comClearFifo(COM_PORT_E _ucPort)
{
	...
	comClearTxFifo(_ucPort);
	
	//停止超时成帧定时器
	bsp_StopTimer(_ucPort);
}
...
void bsp_SetUartBaud(COM_PORT_E _ucPort,uint32_t _baud)
{
  ...
  USART_Cmd(pUart->uart, DISABLE);		/* 禁止串口 */
	...
	*/
	USART_Cmd(pUart->uart, ENABLE);		/* 使能串口 */

	/* CPU的小缺陷：串口配置好，如果直接Send，则第1个字节发送不出去
		如下语句解决第1个字节无法正确发送出去的问题 */
	USART_ClearFlag(pUart->uart, USART_FLAG_TC);     /* 清发送完成标志，Transmission Complete flag */
}
...
static void UartIRQ(UART_T *_pUart)
{
	...
	if (USART_GetITStatus(_pUart->uart, USART_IT_RXNE) != RESET)
	{
		...
		//用于超时成帧,30ms
		bsp_StartTimer(_pUart->ucPort, 30);
